# HALPIS Intercom Basestation for Raspberry Pi
...
from ic_neopixel import IntercomNeopixel
from ic_tca9548a import IntercomTCA9548Multiplexer
from ic_menu_bs import menu
...
def wait_for_mumble(timeout=60, interval=1):
    ...
    while time() - start_time < timeout:
        if is_mumble_window_visible():
            ...
            return True
        render_text_display(index=0,
                            text="WAITING",
                            font_path=intercom.font_path,
                            font_size=intercom.small_font_size)
        render_text_display(index=1,
                            text="FOR",
                            font_path=intercom.font_path,
                            font_size=intercom.small_font_size)
        render_text_display(index=2,
                            text="MUMBLE",
                            font_path=intercom.font_path,
                            font_size=intercom.small_font_size)
        render_text_display(index=3,
                            text="START",
                            font_path=intercom.font_path,
                            font_size=intercom.small_font_size)
        sleep(interval)
...
def connect_mqtt() -> None:
    ...
    try:
        ...
        render_text_display(index=3,
                            text=intercom.mqtt_status_codes[3],
                            font_path=intercom.font_path,
                            font_size=intercom.small_font_size)
        client.loop_stop()
        return
        ...
        render_text_display(index=3,
                            text=intercom.mqtt_status_codes[98],
                            font_path=intercom.font_path,
                            font_size=intercom.small_font_size)
        client.loop_stop()
        return
        ...
        render_text_display(index=3,
                            text=intercom.mqtt_status_codes[99],
                            font_path=intercom.font_path,
                            font_size=intercom.small_font_size)
        client.loop_stop()
        return
...
def on_connect(
    ...
    if reason_code.is_failure:
        ...
        if 'identifier rejected' in str(reason_code).lower():
            logger.warning(f"MQTT Connection Error: "
                           f"{str(reason_code).lower()}")
            render_text_display(index=3,
                                text=intercom.mqtt_status_codes[2],
                                font_path=intercom.font_path,
                                font_size=intercom.small_font_size)
        ...
        if 'bad user name' in str(reason_code).lower():
            logger.warning(f"MQTT Connection Error: "
                           f"{str(reason_code).lower()}")
            render_text_display(index=3,
                                text=intercom.mqtt_status_codes[4],
                                font_path=intercom.font_path,
                                font_size=intercom.small_font_size)
        if 'not authorized' in str(reason_code).lower():
            logger.warning(f"MQTT Connection Error: "
                           f"{str(reason_code).lower()}")
            render_text_display(index=3,
                                text=intercom.mqtt_status_codes[5],
                                font_path=intercom.font_path,
                                font_size=intercom.small_font_size)
    else:
        logger.debug("MQTT connected")
        render_text_display(index=3,
                            text=intercom.mqtt_status_codes[0],
                            font_path=intercom.font_path,
                            font_size=intercom.small_font_size)
        ...
        mqtt_publish(intercom.mqtt_prefix + "status/" +
                    intercom.host_name + "/headset_state",
                    int(intercom.headset))
        mqtt_publish(intercom.mqtt_prefix + "status/" +
                    intercom.host_name + "/device_ip",
                    intercom.device_ip)
        mqtt_publish(intercom.mqtt_prefix + "status/" +
        ...
        mqtt_publish(intercom.mqtt_prefix + "status/" +
                    ...
                    intercom.headset_audio_level)
        mqtt_publish(intercom.mqtt_prefix + "status/" +
                    intercom.host_name + "/speaker_audio_level",
                    intercom.audio_level)
        mqtt_publish(intercom.mqtt_prefix + "status/" +
        ...
        # This is used to track user activity and manage the screensaver
...
def on_disconnect(
    ...
    Args:
        client (Client): The MQTT client instance.
        userdata (None): User-defined data of any type.
        rc (DisconnectFlags): The disconnection result.
        properties (ReasonCode): The properties associated with the 
        disconnection.
        reason_code (Properties): The reason code for the disconnection.
    """
...
def update_talking_status(message: str, talks_to_me: str) -> None:
    ...
    """
    if intercom.user and intercom.user in message:
...
def clear_display(index: int) -> None:
    ...
    """
    multiplexer.select_port(multiplexer.mux_ports[index])
...
def render_text_display(
    ...
    if freeze:
        ...
        if freeze_start != 0 or freeze_start is not None:
...
def on_message(client: Client, userdata: None, 
    ...
    if fnmatch.fnmatch(message.topic,
        ...
        talks_to_me = splitted_topic[len(splitted_topic) - 2].upper()
...
def show_menu(main: int, sub: int, active: int) -> None:
    ...
    """
    multiplexer.select_port(multiplexer.mux_ports[4])
    max_lines = displays[4].get_max_text_lines(
        font_path=intercom.font_path,
        font_size=intercom.small_font_size
    )
    ...
    displays[4].update_freeze = True
    displays[4].update_freeze_start = 0
    displays[4].clear_display()
    displays[4].update_menu(
        lines=lines,
...
def hide_menu() -> None:
    ...
    displays[4].update_freeze = False
    mqtt_publish(intercom.mqtt_prefix
                 + "status/"
                 + intercom.host_name
                 + "/in_menu",
...
def button_long_press_callback(button: IntercomButton) -> None:
    ...
    """
    intercom.last_interaction_time = int(time() * 1000)
    # Only listen for long press on button 8 to enter menu mode
    if button.num == 8 and not intercom.in_menu:
        intercom.in_menu = True
        intercom.menu_main = 0
        intercom.menu_sub = 0
        # Replace IP text in menu with actual IP
        ip = intercom.device_ip
        if len(ip) > 11:
            ip =  ".." + ip[-11:]
        menu[0][6]["name"] = ip
        # Replace DEVICE text with device name
        menu[0][7]["name"] = intercom.host_name
    ...
    if main == 0:
        ...
        if sub == 0:
            ...
            if sub == 2:
                ...
                menu[3][1]["name"] = (
                    "HEADSET ON" if intercom.headset
                    else "HEADSET OFF"
                )
                show_menu_and_set_intercom(menu[0][3], menu[0][3])
            else:
                show_menu_and_set_intercom(menu[0][sub], menu[0][sub])
    elif main == 1:
        # Button lock menu
        if sub == 0:
        ...
        if sub == 0:
            ...
            intercom.headset = not intercom.headset
            GPIO.output(
                11,
                GPIO.HIGH if intercom.headset else GPIO.LOW
            )
            intercom.set_audio_level(
                intercom.headset_audio_level
                if intercom.headset
                else intercom.audio_level
            )
            # Publish headset state to MQTT
            mqtt_publish(intercom.mqtt_prefix
                            + "status/"
                            + intercom.host_name
                            + "/headset_state",
                            str(int(intercom.headset))
                            )
            menu[3][1]["name"] = (
                "HEADSET ON" if intercom.headset
                else "HEADSET OFF"
            )
            show_menu_and_set_intercom(menu[3][1], menu[3][1])
    elif main == 4:
        # Mic level menu
        if sub == 0:
            show_menu_and_set_intercom(menu[4][0], menu[4][0])
        elif sub == 1:
        ...
        # Shutdown menu
        if sub == 0:
            show_menu_and_set_intercom(menu[5][0], menu[5][0])
        elif sub == 1:
...
def button_change_callback(button: IntercomButton) -> None:
    ...
    """
    intercom.last_interaction_time = int(time() * 1000)
    if intercom.lock_buttons[button.num] is False:
    ...
    if button.is_pressed:
        if button.num == 8:
            ...
            # Buttons 0-7
            neopixels.set_neopixel_color(button.num, "red")
            publish_talking_to()
        ...
        if button.num == 8:
            ...
            if (intercom.menu_main == 4 and intercom.menu_sub == 1
                    and not intercom.encoder_adjust):
                intercom.encoder_adjust = True
            elif (intercom.menu_main == 4 and intercom.menu_sub == 1
            ...
            if not button.is_locked:
                neopixels.set_neopixel_off(button.num)
                publish_talking_to()
...
def initialize_displays(i2c_port: int = 1) -> Dict[int, IntercomDisplay]:
    ...
    # Initialize displays for each multiplexer port, using an index
    for index, port in enumerate(multiplexer.mux_ports):
        # Select the correct multiplexer port
        multiplexer.select_port(port)
        ...
        # Optional: clear the multiplexer port after initializing the display
        multiplexer.clear_port()
...
def clear_buttons() -> None:
    """
    Turn off all neopixels and unlock all buttons.
    """
    for i in range(neopixels.count):
        neopixels.set_neopixel_off(i)
    # Unlock all buttons
    for i in buttons:
        buttons[i].is_locked = False
    if IntercomButton.buttons_enabled:
        # Broadcast not talking    
        mqtt_publish(intercom.mqtt_prefix + "broadcast/" +
                    intercom.user.lower() + "/talk", "not talking")
            ...
            if value > intercom.encoder_last:
                ...
                if intercom.menu_sub + 1 <= sub_max:
                    ...
                    show_menu(main=intercom.menu_main,
                              sub=intercom.menu_sub, 
                              active=intercom.menu_sub)
            else:
...
def render_screensaver(display: int, text: str) -> None:
    """
    Render the screensaver on a specified display.

    Args:
        display_index (int): The index of the display to render the 
                            screensaver on.
        user_text (str): The text to display on the screensaver.
    """
    multiplexer.select_port(multiplexer.mux_ports[display])
    displays[display].render_screensaver(
        user=text,
        screensaver_brightness=intercom.screensaver_brightness,
        font_path=intercom.font_path,
    )

def display_control(
    current_time: int, 
    last_screensaver_update: int, 
    ...
    Args:
        ...
        last_screensaver_update (int): The last time the screensaver 
                                    was updated.
        screensaver_active (bool): Flag indicating if the screensaver 
                                    is active.

    Returns:
        Tuple[int, bool]: Updated last_screensaver_update and 
                                    screensaver_active.
    """
    ...
    if not intercom.in_menu:
        for i in displays:
            ...
            current_text = intercom.talk_to[i]

            if not current_display.update_freeze:
                time_exceeded = (
                    current_time - intercom.last_interaction_time
                ) >= (intercom.screensaver_delay * 1000)
                if intercom.screensaver and time_exceeded:
                    screensaver_active = True
                else:
                    if screensaver_active:
                        screensaver_active = False
                        # Clear text to force update
                        for id in displays:
                            displays[id].text = ""

                # If role is not set, show "Pending Setup"
                if intercom.user == "NO SETUP":
                    if i == 1:
                ...
                # Check if the display text needs to be updated
                if current_display.text != current_text or screensaver_active:
                    if "," in current_text:
                        # Time-based split update
                        if (
                            current_display.last_text_split
                            ...
                            if screensaver_active:
                                # If screensaver is active, render screensaver
                                if elapsed_time >= SCREENSAVER_INTERVAL_MS:
                                    # Screensaver on
                                    intercom.screensaver_activated = True
                                    last_screensaver_update = current_time
                                    render_screensaver(i, 
                                                       text_split[text_index])
                                    cd = current_display
                                    cd.text = text_split[text_index]
                                    cd.last_text_split = current_time
                            else:
                                intercom.screensaver_activated = False
                                # Update the display text
                                port = multiplexer.mux_ports[i]
                                multiplexer.select_port(port)
                                render_text_display(
                                    index=i,
                                    text=text_split[text_index],
                                    font_path=intercom.font_path,
                                    font_size=intercom.normal_font_size,
                                )
                                current_display.text = text_split[text_index]
                        ...
                        if screensaver_active:
                            ...
                            if elapsed_time >= SCREENSAVER_INTERVAL_MS:
                                ...
                                intercom.screensaver_activated = True
                                last_screensaver_update = current_time
                                render_screensaver(i, current_text)
                        else:
                            # Direct update if no split is needed
                            intercom.screensaver_activated = False
                            multiplexer.select_port(multiplexer.mux_ports[i])
                            render_text_display(
                                index=i,
                                text=current_text,
                                font_path=intercom.font_path,
                        ...
                        current_display.text = current_text
            else:
                if current_display.update_freeze_start > 0:
                    if (
                        current_display.update_freeze_start
                        + current_display.update_freeze_time_ms
                        <= current_time
                    ):
                        current_display.update_freeze = False
    return last_screensaver_update, screensaver_active

def who_talks_to_me(current_time: int) -> list[str]:
    """
    Indicate who is talking to me.

    Args:
        current_time (int): The current time in milliseconds.

    Returns:
        list[str]: A list of active keys indicating who is talking to me.
    """
    active_keys = [
        key for key, value in intercom.talking_to_me.items()
        if (value[1]
            or (not value[1]
                and (current_time - value[0]
                     < intercom.neopixel_post_talk_blink_ms))
            )
    ]

    # Check if any key starts with "CAM" and add "CAMS" to 
    # active_keys if not already in active_keys
    if (any(key.upper().startswith("CAM") for key in active_keys)
            and not any(key.upper() == "CAMS" for key in active_keys)):
        active_keys.append("CAMS")
...
def system_control():
    ...
    while intercom.system_control:
        ...
        # Display control
        last_screensaver_update, screensaver_active = display_control(
            current_time=current_time,
            last_screensaver_update=last_screensaver_update,
            screensaver_active=screensaver_active
            )  

        # Neopixel control
        active_keys = who_talks_to_me(current_time)

        # Global timer for synchronized blinking
        if last_color_switch + intercom.neopixel_blink_rate_ms < current_time:
            # Toggles the global blink state (True/False)
            blink_toggle = not blink_toggle
            last_color_switch = int(time() * 1000)

        if active_keys:
            for user in active_keys:
                index = next(
                    (key for key, value in intercom.talk_to.items()
                     if value == user), None)
                if index is not None:
                    # Are we talking to the one(s) talking to me?
                    if buttons[index].is_pressed or buttons[index].is_locked:
                        # Global blink control: toggle between red and yellow
                        if blink_toggle:
                            # Check if already yellow
                            if (neopixels.led_pixels[index] !=
                                    neopixels.colors['yellow']):
                                neopixels.set_neopixel_color(index, "yellow")
                        else:
                            # Check if already red
                            if (neopixels.led_pixels[index] !=
                                    neopixels.colors['red']):
                                neopixels.set_neopixel_color(index, "red")
                    else:
                        # Global blink control: toggle between yellow and off
                        if blink_toggle:
                            # Check if already yellow
                            if (neopixels.led_pixels[index] !=
                                    neopixels.colors['yellow']):
                            ...
                            if (neopixels.led_pixels[index] !=
                                    neopixels.colors['off']):
                                neopixels.set_neopixel_off(index)

        # Who has stopped talking in the last neopixel_post_talk_blink_ms?
        inactive_keys = [
            key for key, value in intercom.talking_to_me.items()
            if (not value[1] and (current_time - value[0] >
                                  intercom.neopixel_post_talk_blink_ms))
        ]

        if inactive_keys != previous_inactive_keys:
            if inactive_keys:
                for user in inactive_keys:
                    index = next(
                        (key for key, value in intercom.talk_to.items()
                         if value == user), None)
                    if index is not None:
                        # Are we talking to the one(s) that
                        # just stopped talking to me?
                        if (buttons[index].is_pressed
                                or buttons[index].is_locked):
                            neopixels.set_neopixel_color(index, "red")
                        else:
                            neopixels.set_neopixel_off(index)

            previous_inactive_keys = inactive_keys.copy()

        for i in range(neopixels.count):
            # Turn off any yellow LEDs that are not listed in active_keys 
            # or inactive_keys
            if neopixels.led_pixels[i] == neopixels.colors['yellow']:
                if intercom.talk_to[i] not in active_keys + inactive_keys:
                    neopixels.set_neopixel_off(i)
            
            # Set LEDs to red if the corresponding button is locked or 
            # pressed, and currently off
            if (buttons[i].is_locked or buttons[i].is_pressed):
                if (neopixels.led_pixels[i] == neopixels.colors["off"]):
                    neopixels.set_neopixel_color(i, "red")
...
def cleanup():
    """
    Control the system operation.
    This function manages the system control loop, handling display updates,
    neopixel control, and button interactions. It operates at a defined update
    interval and ensures synchronized blinking and state changes for the
    neopixels based on user interactions and system status.
    """
    if 'intercom' in globals():
        update_config = 0
        audio_level = intercom.get_audio_level()
        mic_level = intercom.get_mic_level()
        if audio_level != config.audio_level and not intercom.headset:
        ...
        if config.mic_level != mic_level:
            if 0 < mic_level <= 150:
                ...
                update_config = 1
        if config.headset != intercom.headset:
            # Headset has changed, update config
            config.headset = intercom.headset
            update_config = 1
        if config.headset_audio_level != intercom.headset_audio_level:
            if 0 < intercom.headset_audio_level <= 150:
                # Headset audi level has changed, update config
                config.headset_audio_level = intercom.headset_audio_level
                update_config = 1
        ...
        if intercom.system_active:
            ...
            if len(stack) > 1:
                ...
                logger.info(
                    "Cleanup called without a valid caller in the stack.")
...
if __name__ == "__main__":
    global buttons, client, config, displays, intercom, \
        multiplexer, neopixels
    ...
    config_file_path = os.path.join(current_dir, "config_bs.json5")
    ...
    intercom = Intercom(
        ...
        headset_audio_level=config.headset_audio_level,
        ...
        neopixel_brightness=config.np_brightness,
        neopixel_blink_rate_ms=config.np_blink_rate_ms,
        neopixel_post_talk_blink_ms=config.np_post_talk_blink_ms,
    ...
    if intercom.headset:
        ...
        intercom.set_audio_level(intercom.headset_audio_level)
        intercom.audio_level = config.audio_level
    else:
        # Speaker activated
    ...
    #mux_ports = [0x80, 0x40, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20]
    mux_ports = [0x40, 0x80, 0x01, 0x02, 0x20, 0x10, 0x08, 0x04] #hw v2.3
    multiplexer = IntercomTCA9548Multiplexer(
        i2c_expander_address=0x70, i2c_port=1, mux_ports=mux_ports)

    # Displays
    displays = initialize_displays()

    render_text_display(1, "HALPIS",
                        intercom.font_path, intercom.small_font_size)
    
    render_text_display(2, "INTCM",
                        intercom.font_path, intercom.small_font_size)
    
    render_text_display(5, "BASESTATION",
                        intercom.font_path, intercom.small_font_size)
    
    render_text_display(6, "INITIALIZING",
                        intercom.font_path, intercom.small_font_size)

    sleep(3)

    render_text_display(0, intercom.host_name,
                        intercom.font_path, intercom.small_font_size)
    render_text_display(1, intercom.version,
                        intercom.font_path, intercom.small_font_size)

    # Neopixel
    neopixel_count = 8
    neopixels = IntercomNeopixel(
        0, 0, neopixel_count, intercom.neopixel_brightness)

    # Initialize GPIO
    GPIO.setmode(GPIO.BOARD)
    # 13, 15, 19, 29
    # Define the GPIO pins for the buttons
    #button_pins = [31, 32, 33, 35, 36, 37, 38, 40, 29]
    button_pins = [40, 37, 33, 32, 38, 36, 35, 31, 29] # hw v2.3
    ...
    client.username_pw_set(intercom.mqtt_user, password=intercom.mqtt_pass)
    ...
    try:
        while intercom.system_active:
            if intercom.device_ip == "":
                render_text_display(
                    2, "NO IP", intercom.font_path, intercom.small_font_size)
                intercom.device_ip = get_ip()
                render_text_display(index=2,
                                    text=intercom.device_ip,
                                    font_path=intercom.font_path,
                                    font_size=intercom.small_font_size
                                    )
            # IS MQTT CONNECTED?
